...
st.title('Statistik Pariwisata :blue[Kabupaten Pangandaran]')
...
with kol1a:
    with st.container(border=True):
        st.success('Tingkat Penghunian Kamar (TPK) Hotel Kabupaten Pangandaran')
...
with kol1b:
    with st.container(border=True):
        st.info('Tingkat Pemakaian Tempat Tidur (TPTT) Hotel Kabupaten Pangandaran')
...
with st.expander('Rata-rata Lama Menginap Tamu'):        
    ...
    with kol2a:
        with st.container(border=True):
            st.success('Rata-rata Lama Menginap Tamu Asing (RLMTA) di Hotel Kabupaten Pangandaran')
    ...
    with kol2b:
        with st.container(border=True):
            st.info('Rata-rata Lama Menginap Tamu (RLMT) di Hotel Kabupaten Pangandaran')
    ...
    with kol2c:
        with st.container(border=True):
            st.success('Rata-rata Lama Menginap Tamu Dalam Negeri (RLMTD) di Hotel Kabupaten Pangandaran')
...
with st.expander('Jumlah Tamu'):
    ...
    with kol3a:
        with st.container(border=True):
            st.success('Jumlah Tamu Asing - Hotel Kabupaten Pangandaran')
    ...
    with kol3b:
        with st.container(border=True):
            st.info('Jumlah Tamu Dalam Negeri - Hotel Kabupaten Pangandaran')
...
with st.expander('Lihat Tabel Lengkap'):
    st.warning('Statistik Pariwisata Kabupaten Pangandaran')
