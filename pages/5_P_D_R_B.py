...
st.title('Produk Domestik Regional Bruto (PDRB) :blue[Kabupaten Pangandaran]')
...
pdrblu = data.sort_values(by=['Tahun', 'Lapangan Usaha'], ascending=[False, True])
...
with st.container(border=True):
    st.success('Tren PDRB Kabupaten Pangandaran (Milyar Rupiah)')
...
with tab1a:
    with st.container(border=True):
        st.success('Tren PDRB Berlaku menurut Lapangan Usaha Kabupaten Pangandaran (Milyar Rupiah)')
...
with tab1b:
    with st.container(border=True):
        st.success('Tren PDRB Konstan menurut Lapangan Usaha Kabupaten Pangandaran (Milyar Rupiah)')
...
with tab2a:
    with st.container(border=True):
        st.success('Tren PDRB Berlaku menurut Pengeluaran Kabupaten Pangandaran (Milyar Rupiah)')
...
with tab2b:
    with st.container(border=True):
        st.success('Tren PDRB Konstan menurut Pengeluaran Kabupaten Pangandaran (Milyar Rupiah)')
...
if tahun_terpilih:
    ...
    with kol2a:
        with st.container(border=True):
            ...
            pdrbx_terpilih = pdrbx_terpilih[pdrbx_terpilih['Kelompok'] != 'Total PDRB']
...
with st.expander('Lihat Tabel Lengkap PDRB menurut Lapangan Usaha'):
    st.warning('Produk Domestik Regional Bruto menurut Lapangan Usaha Kabupaten Pangandaran')
    df = data.sort_values(by='Tahun', ascending=False)
    st.dataframe(df, hide_index=True, use_container_width=True)
    
with st.expander('Lihat Tabel Lengkap PDRB menurut Pengeluaran'):
    st.warning('Produk Domestik Regional Bruto menurut Pengeluaran Kabupaten Pangandaran')
