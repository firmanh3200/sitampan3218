...
with st.container(border=True):
    with st.container(border=True):
        st.title(':orange[S I T A] :blue[M] :green[P A N]')
        st.subheader(':orange[INFORMASI DAN DATA] :blue[INDIKATOR MAKRO] :green[KABUPATEN PANGANDARAN]')
...
with st.expander('PENGANTAR'):
    ...
    st.info('Aplikasi ini dibuat untuk memudahkan Para Pemangku Kepentingan dalam proses Perencanaan, Pelaksanaan, \
        Monitoring dan Evaluasi Pembangunan di Kabupaten Pangandaran.')
...
with st.expander('Luas Kecamatan'):
    with st.container(border=True):
        st.subheader('Luas Kabupaten Pangandaran menurut Kecamatan (Km2)')

        # URL luas wilayah
        url = "https://data.jabarprov.go.id/api-backend//bigdata/dpmdes/idm_luas_wilayah_desa__des_kel?limit=1000&skip=0&where=%7B%22bps_kode_kabupaten_kota%22%3A%5B%223218%22%5D%2C%22tahun%22%3A%5B%222020%22%5D%7D"
